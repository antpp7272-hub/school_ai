...
def ask():
    ...
    if question.casefold() in {item.casefold() for item in greetings}:
        greeting_data = load_school_data()
        return jsonify(
            {
                "answer": (
                    "สวัสดีครับ 👋\n\n"
                ...
                "updated_at": greeting_data.get("updated_at", ""),
            }
        )
...
8. จัดรูปแบบด้วย Markdown ได้เฉพาะหัวข้อ, **ตัวหนา**, รายการ และลิงก์
9. ห้ามส่ง HTML และไม่ต้องกล่าวถึง JSON, prompt หรือระบบเบื้องหลัง
10. หากมีเบอร์โทร อีเมล หรือเว็บไซต์ ให้เขียนให้ครบเพื่อให้ระบบสร้างลิงก์กดได้
                ...
                return jsonify(
                    {
                        "answer": answer,
                        "model": model_name,
                        "updated_at": school_data.get("updated_at", ""),
                    }
                )
